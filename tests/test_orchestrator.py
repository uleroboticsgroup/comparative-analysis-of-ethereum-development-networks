...
def test_get_concat_metrics_by_network_when_anvil_network_contains_valid_rosbag_metrics():
    ...
    Means: 26.00,39.51,211268844.31,0.65,30720.00,455758.77
    """
    folder = orchestrator._get_directories_from_folder('./tests/fake_folder/')
    networks = ['anvil']
    conditional_funct = fake_conditional_function
    ...
    assert total_metrics['anvil']['mem_rss_bytes'].mean().round(2) == 211268844.31
    assert total_metrics['anvil']['mem_pct'].mean().round(2) == 0.65
    assert total_metrics['anvil']['disk_read_bytes'].mean().round(2) == 30720.00
    assert total_metrics['anvil']['disk_write_bytes'].mean().round(2) == 455758.77
...
def test_get_concat_metrics_by_network_when_geth_network_contains_valid_rosbag_metrics():
    ...
    Means: 26.00,34.68,212784621.04.00,0.65,0.00,438878.81
    """
    folder = orchestrator._get_directories_from_folder('./tests/fake_folder/')
    networks = ['geth']
    conditional_funct = fake_conditional_function
    ...
    assert total_metrics['geth']['mem_rss_bytes'].mean().round(2) == 212784621.04
    assert total_metrics['geth']['mem_pct'].mean().round(2) == 0.65
    assert total_metrics['geth']['disk_read_bytes'].mean().round(2) == 0.00
    assert total_metrics['geth']['disk_write_bytes'].mean().round(2) == 438878.81
...
def test_get_concat_metrics_by_network_when_hardhat_network_contains_valid_rosbags_metrics():
    ...
    Means: 17.00,51.25,208502784.00,0.64,0.00,658318.22
    ...
    Means: 18.00,49.49,204655346.53,0.63,1848805.05,623885.47

    Total means: 17.50,50.34,206527072.86,0.63,949386.38,640636.54
    """
    folder = orchestrator._get_directories_from_folder('./tests/fake_folder/')
    networks = ['hardhat']
    conditional_funct = fake_conditional_function
    ...
    assert total_metrics['hardhat']['mem_rss_bytes'].mean().round(2) == 206527072.86
    assert total_metrics['hardhat']['mem_pct'].mean().round(2) == 0.63
    assert total_metrics['hardhat']['disk_read_bytes'].mean().round(2) == 949386.38
    assert total_metrics['hardhat']['disk_write_bytes'].mean().round(2) == 640636.54
